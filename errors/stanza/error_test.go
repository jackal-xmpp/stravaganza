...
func TestStanzaError_Element(t *testing.T) {
	...
	// then
	require.Equal(t, `<message from="ortuman@jackal.im/balcony" to="noelia@jackal.im/yard" type="error"><body>Hi everyone!</body><error code="500" type="wait"><internal-server-error xmlns="urn:ietf:params:xml:ns:xmpp-stanzas"/><text xmlns="urn:ietf:params:xml:ns:xmpp-stanzas" xml:lang="es">Error interno de servidor</text><app-specific xmlns="app-ns"/></error></message>`, seElem.String())
}
