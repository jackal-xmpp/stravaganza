...
type Error struct {
	r          Reason
	senderElem stravaganza.Element
...
// Reason returns stanza error reason type.
func (se *Error) Reason() Reason {
	return se.r
}
...
// Element returns stanza error XMPP element.
func (se *Error) Element() stravaganza.Element {
	return stravaganza.NewBuilderFromElement(se.senderElem).
		WithAttribute(stravaganza.Type, stravaganza.ErrorType).
		WithAttribute(stravaganza.From, se.senderElem.Attribute(stravaganza.To).Value).
		WithAttribute(stravaganza.To, se.senderElem.Attribute(stravaganza.From).Value).
...
func (se *Error) errSubElement() stravaganza.Element {
	...
	b.WithAttribute("code", strconv.Itoa(se.r.Code()))
	b.WithAttribute("type", se.r.Type().String())
	b.WithChild(
...
func E(reason Reason, senderElem stravaganza.Element) *Error {
	return &Error{r: reason, senderElem: senderElem}
}
